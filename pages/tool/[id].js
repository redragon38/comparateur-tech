...
export default function ToolPage({ tool, alts }) {
  ...
  const schemas = [
    ...
    buildBreadcrumbSchema([
      ...
      ...(cat ? [{ name: cat, url: `/outils/${catSlug}` }] : []),
      { name: tool.name },
    ]),
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO
        ...
        description={tool.short ? `${tool.short.slice(0,140)} Avis expert, prix, alternatives.` : `Découvrez notre avis sur ${tool.name} : fonctionnalités, prix, points forts et alternatives.`}
        canonical={`https://comparateur-tech.com/tool/${tool.id}`}
        ogType="article"
        keywords={`${tool.name}, avis ${tool.name}, ${tool.name} prix, alternative ${tool.name}, ${(tool.categories||[]).join(', ')}`}
