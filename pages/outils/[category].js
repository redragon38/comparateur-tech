...
import { USECASE_PAGES, getAlternativePath, getComparisonPath } from '../../lib/programmatic-seo';
...
export async function getStaticProps({ params }) {
  ...
  return {
    props: { tools: tools.map(toCardTool), meta, slug, allCategories },
  };
}
...
export default function CategoryPage({ tools, meta, slug, allCategories }) {
  ...
  const matchingUsecases = USECASE_PAGES.filter(page => page.category === meta.label).slice(0, 4);
  ...
  return (
    <>
      <SEO
        ...
        description={`Comparez les meilleurs ${meta.label.toLowerCase()} : avis éditoriaux, prix, essais gratuits et classement ${new Date().getFullYear()}. ${tools.length} outils sélectionnés et vérifiés.`}
          ...
          <div className="container mx-auto px-6 pb-32">
            <div className="flex flex-col lg:flex-row gap-10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <section className="bg-white rounded-2xl border border-purple-100 shadow-sm p-6 mb-8">
                  <h2 className="text-xl font-bold text-gray-900 mb-5">À comparer dans {meta.label}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {matchingUsecases.map(page => (
                    ...
                    {topTools.slice(0, 3).map(tool => (
                      <Link key={`alt-${tool.id}`} href={getAlternativePath(tool)} className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 hover:border-purple-300 hover:bg-purple-50 transition-colors">
                        <p className="text-sm font-semibold text-gray-900">Alternatives à {tool.name}</p>
                      </Link>
                    ))}
                    {topTools.slice(0, 3).map((tool, index) => topTools[index + 1] ? (
                      <Link key={`cmp-${tool.id}`} href={getComparisonPath(tool, topTools[index + 1])} className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 hover:border-purple-300 hover:bg-purple-50 transition-colors">
                        <p className="text-sm font-semibold text-gray-900">{tool.name} vs {topTools[index + 1].name}</p>
                      </Link>
                    ) : null)}
                  </div>
                </section>
